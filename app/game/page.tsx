...
const GamePage: React.FC = () => {
  ...
  /** Same as clicking Play on the default room: instant /board + fresh WS (no full reload). */
  const redirectToDefaultRoomBoard = useCallback(() => {
    rejoinDefaultRoomBoard();
    router.push(`/board?room=${encodeURIComponent(S1_DEFAULT_ROOM_ID)}`);
  }, [rejoinDefaultRoomBoard, router]);

  const handleClose = useCallback(() => {
    sessionStorage.removeItem("hasReloaded");
    redirectToDefaultRoomBoard();
  }, [redirectToDefaultRoomBoard]);
